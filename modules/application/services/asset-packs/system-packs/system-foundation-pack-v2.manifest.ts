...
export const SYSTEM_FOUNDATION_PACK_V2_MANIFEST: AssetPackManifest = {
  ...
  metadata: {
    ...
    catalogVersion: SYSTEM_FOUNDATION_V2_PACK_VERSION,
...
function replaceFoundationVersion<T>(value: T): T {
  if (value === SYSTEM_FOUNDATION_PACK_VERSION) {
    return SYSTEM_FOUNDATION_V2_PACK_VERSION as T;
  }
...
export function readSystemFoundationV2OrEarlierManifest(
  version: string,
...
export function exactSystemFoundationV2DefinitionReference(
  definitionId: string,
  version = SYSTEM_FOUNDATION_V2_PACK_VERSION,
): AssetReference {
  const manifest = readSystemFoundationV2OrEarlierManifest(version);
