...
export interface RegisterArtifactRepoApiRoutesDependencies {
  ...
  getHuggingFaceTokenStatus: () => MaybePromise<{
    configured: boolean;
    maskedToken?: string;
  }>;
  setHuggingFaceToken: (
    token: string,
  ) => MaybePromise<{ configured: boolean; maskedToken?: string }>;
  clearHuggingFaceToken: () => MaybePromise<{
    configured: boolean;
    maskedToken?: string;
  }>;
...
function normalizeSource(value: string | undefined): string {
  ...
  return normalized && normalized.length > 0
    ? normalized
    : "thin-client.artifact-repo";
}

function mapRequestContext(request: ArtifactRepoExpressRequestLike): {
  requestId?: string;
...
function mapApiStoreRequestToCommand(
  ...
  const apiRequest = createApiArtifactRepoStoreRequest(
    {
      ...
      repositoryCreation: parseRepositoryCreation(
        requestBody.repositoryCreation,
      ),
      boundary: {
        host: "server",
        source: normalizeSource(requestBody.source),
  ...
  return {
    ...
    content: new Uint8Array(
      Buffer.from(apiRequest.payload.contentBase64, "base64"),
    ),
...
function parseRepositoryCreation(
  value:
    | {
        approved?: boolean;
        visibility?: string;
    ...
    | undefined,
): { approved: true; visibility: "private" | "public" } | undefined {
...
function mapStatusCode(
  response: ApiArtifactRepoHasResponse | ApiArtifactRepoStoreResponse,
): number {
  if (response.ok) {
    return 200;
  }
...
export function registerArtifactRepoApiRoutes(
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      try {
        const token =
          typeof (request.body as { token?: unknown } | undefined)?.token ===
          "string"
          ...
          error: {
            ...
            message:
              error instanceof Error
                ? error.message
                : "Invalid Hugging Face token request.",
          },
        };
  ...
  dependencies.app.post("/api/artifact-repo/has", async (request, response) => {
    ...
    try {
      command = mapApiHasRequestToCommand(
        request.body as ArtifactRepoHasApiRequestBody,
        context,
      );
    } catch (error) {
      const apiResponse = createApiArtifactRepoHasFailureResponse(
        ...
        error instanceof Error
          ? error.message
          : "Invalid artifact-repo has request.",
        context,
      );
    ...
    const result = await dependencies.hasArtifactInRepoUseCase.execute(
      command,
      context,
    );
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      const context = mapRequestContext(request);
      let apiRequest: ReturnType<
        typeof createApiHuggingFaceNamespaceDatasetsBrowseRequest
      >;
      ...
      try {
        apiRequest = createApiHuggingFaceNamespaceDatasetsBrowseRequest(
          {
            namespace:
              (request.body as { namespace?: string } | undefined)?.namespace ??
              "",
            source: normalizeSource(
              (request.body as { source?: string } | undefined)?.source,
            ),
          },
          context,
        );
        ...
        const apiResponse =
          createApiHuggingFaceNamespaceDatasetsBrowseFailureResponse(
            ...
            error instanceof Error
              ? error.message
              : "Invalid Hugging Face namespace browse request.",
            context,
          );
      ...
      const result =
        await dependencies.browseHuggingFaceNamespaceDatasetsUseCase.execute(
          {
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      const context = mapRequestContext(request);
      let apiRequest: ReturnType<
        typeof createApiHuggingFaceDatasetParquetFilesBrowseRequest
      >;
      ...
      try {
        apiRequest = createApiHuggingFaceDatasetParquetFilesBrowseRequest(
          {
            repository:
              (request.body as { repository?: string } | undefined)
                ?.repository ?? "",
            revision: (request.body as { revision?: string } | undefined)
              ?.revision,
            source: normalizeSource(
              (request.body as { source?: string } | undefined)?.source,
            ),
        ...
        const apiResponse =
          createApiHuggingFaceDatasetParquetFilesBrowseFailureResponse(
            ...
            error instanceof Error
              ? error.message
              : "Invalid Hugging Face dataset file browse request.",
            context,
          );
        response.status(mapBrowseStatusCode(apiResponse)).json(apiResponse);
        return;
      ...
      const result =
        await dependencies.browseHuggingFaceDatasetParquetFilesUseCase.execute(
          {
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      ...
      try {
        command = mapApiStoreRequestToCommand(
          request.body as ArtifactRepoStoreApiRequestBody,
          context,
        );
      } catch (error) {
        const apiResponse = createApiArtifactRepoStoreFailureResponse(
          ...
          error instanceof Error
            ? error.message
            : "Invalid artifact-repo store request.",
          context,
        );
        response.status(mapStatusCode(apiResponse)).json(apiResponse);
        return;
      ...
      const result = await dependencies.storeArtifactInRepoUseCase.execute(
        command,
        context,
      );
  ...
  dependencies.app.post("/api/artifact/publish", async (request, response) => {
    ...
    try {
      apiRequest = createApiArtifactPublishRequest(
        {
          artifactId: (request.body as ArtifactPublishApiRequestBody)
            .artifactId,
          target: {
            provider:
              (request.body as ArtifactPublishApiRequestBody).target
                ?.provider ?? "",
            repository:
              (request.body as ArtifactPublishApiRequestBody).target
                ?.repository ?? "",
            revision: (request.body as ArtifactPublishApiRequestBody).target
              ?.revision,
            path:
              (request.body as ArtifactPublishApiRequestBody).target?.path ??
              "",
          },
          ...
          source: normalizeSource(
            (request.body as ArtifactPublishApiRequestBody).source,
          ),
        },
        context,
      );
      ...
      const apiResponse = createApiArtifactPublishFailureResponse(
        ...
        error instanceof Error
          ? error.message
          : "Invalid artifact publish request.",
        context,
      );
    ...
    async (request, response) => {
      ...
      try {
        apiRequest = createApiArtifactPublishVerifyRequest(
          {
            artifactId: (request.body as ArtifactPublishVerifyApiRequestBody)
              .artifactId,
            source: normalizeSource(
              (request.body as ArtifactPublishVerifyApiRequestBody).source,
            ),
          },
          context,
        );
        ...
        const apiResponse = createApiArtifactPublishVerifyFailureResponse(
          ...
          error instanceof Error
            ? error.message
            : "Invalid artifact publish-verify request.",
          context,
        );
        response.status(mapPublishStatusCode(apiResponse)).json(apiResponse);
        return;
      ...
      const result =
        await dependencies.verifyPublishedArtifactBackingUseCase.execute({
          artifactId: apiRequest.payload.artifactId,
        });
    ...
    async (request, response) => {
      ...
      try {
        apiRequest = createApiArtifactSourceVerifyRequest(
          {
            artifactId: (request.body as ArtifactSourceVerifyApiRequestBody)
              .artifactId,
            source: normalizeSource(
              (request.body as ArtifactSourceVerifyApiRequestBody).source,
            ),
          },
          context,
        );
        ...
        const apiResponse = createApiArtifactSourceVerifyFailureResponse(
          ...
          error instanceof Error
            ? error.message
            : "Invalid artifact source-verify request.",
          context,
        );
        response.status(mapPublishStatusCode(apiResponse)).json(apiResponse);
        return;
      ...
      const result =
        await dependencies.verifyImportedArtifactSourceBackingUseCase.execute({
          artifactId: apiRequest.payload.artifactId,
        });
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      const context = mapRequestContext(request);
      let apiRequest: ReturnType<
        typeof createApiArtifactRegisterFromRepoRequest
      >;
        ...
        apiRequest = createApiArtifactRegisterFromRepoRequest(
          {
            target: {
              provider:
                (request.body as ArtifactRegisterFromRepoApiRequestBody).target
                  ?.provider ?? "",
              repository:
                (request.body as ArtifactRegisterFromRepoApiRequestBody).target
                  ?.repository ?? "",
              revision: (request.body as ArtifactRegisterFromRepoApiRequestBody)
                .target?.revision,
              path:
                (request.body as ArtifactRegisterFromRepoApiRequestBody).target
                  ?.path ?? "",
            ...
            artifactFamily:
              (request.body as ArtifactRegisterFromRepoApiRequestBody)
                .artifactFamily ?? "image",
            mediaType: (request.body as ArtifactRegisterFromRepoApiRequestBody)
              .mediaType,
            source: normalizeSource(
              (request.body as ArtifactRegisterFromRepoApiRequestBody).source,
            ),
          },
          context,
        );
  ...
  dependencies.app.post(
    ...
    async (request, response) => {
      ...
      let apiRequest: ReturnType<
        typeof createApiArtifactLocalizeFromRepoRequest
      >;
      ...
      try {
        apiRequest = createApiArtifactLocalizeFromRepoRequest(
          {
            workspaceId:
              (request.body as ArtifactLocalizeFromRepoApiRequestBody)
                .workspaceId ?? "",
            artifactId:
              (request.body as ArtifactLocalizeFromRepoApiRequestBody)
                .artifactId ?? "",
            source: normalizeSource(
              (request.body as ArtifactLocalizeFromRepoApiRequestBody).source,
            ),
          },
          context,
        );
        ...
        const apiResponse = createApiArtifactLocalizeFromRepoFailureResponse(
          ...
          error instanceof Error
            ? error.message
            : "Invalid artifact localize-from-repo request.",
          context,
        );
        response.status(mapPublishStatusCode(apiResponse)).json(apiResponse);
        return;
