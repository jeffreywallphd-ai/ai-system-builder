...
import type { DatasetPreparationSummary, DatasetPreparationWarning, DatasetQualityReport, DatasetQualityRequestedConfig, PrepareTrainingDatasetRequest } from "../runtime";
...
export const DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_OPERATION = createTransportOperation("artifact", "prepare-training-dataset.approve");
...
export const DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_REQUEST_CHANNEL = createIpcChannel(DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_OPERATION, "request");
export const DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_RESPONSE_CHANNEL = createIpcChannel(DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_OPERATION, "response");

export interface DesktopDatasetPreparationBoundaryContext { host: "desktop"; source: string; workspaceId?: string; }
export interface DesktopPrepareTrainingDatasetStartRequestPayload { command: { sourceArtifactIds: string[]; recipe: PrepareTrainingDatasetRequest["recipe"]; split: PrepareTrainingDatasetRequest["split"]; output: PrepareTrainingDatasetRequest["output"]; quality?: DatasetQualityRequestedConfig; }; boundary: DesktopDatasetPreparationBoundaryContext; }
...
export interface DesktopPrepareTrainingDatasetApproveRequestPayload { requestId: string; reportFingerprint: string; boundary: DesktopDatasetPreparationBoundaryContext; }
...
export interface DesktopPrepareTrainingDatasetFinalResult {
  ...
  review?: { state: "review-required" | "approved"; reportFingerprint: string; approvalAllowed: boolean };
...
export type DesktopPrepareTrainingDatasetTaskReadSuccessValue =
  ...
  | { status: "succeeded" | "review-required"; requestId: string; taskType?: string; result: DesktopPrepareTrainingDatasetFinalResult; startedAt?: string; updatedAt?: string; completedAt?: string }
...
export interface DesktopPrepareTrainingDatasetApproveSuccessValue { requestId: string; taskType: "prepare-training-dataset"; status: "succeeded"; result: DesktopPrepareTrainingDatasetFinalResult; }
...
export type DesktopPrepareTrainingDatasetApproveRequest = IpcRequest<DesktopPrepareTrainingDatasetApproveRequestPayload, typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_OPERATION, Record<string, never>, typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_REQUEST_CHANNEL.value>;
export type DesktopPrepareTrainingDatasetApproveResponse = IpcResponse<DesktopPrepareTrainingDatasetApproveSuccessValue, Record<string, unknown>, typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_OPERATION, Record<string, never>, typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_RESPONSE_CHANNEL.value>;
...
const b = (boundary: DesktopDatasetPreparationBoundaryContext) => ({ host: "desktop" as const, source: norm(boundary.source, "boundary.source"), ...(boundary.workspaceId ? { workspaceId: norm(boundary.workspaceId, "boundary.workspaceId") } : {}) });
...
export function createDesktopPrepareTrainingDatasetApproveRequest(payload: DesktopPrepareTrainingDatasetApproveRequestPayload, options?: { requestId?: string; correlationId?: string }): DesktopPrepareTrainingDatasetApproveRequest { return createIpcRequest(DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_REQUEST_CHANNEL, { requestId: norm(payload.requestId, "requestId"), reportFingerprint: norm(payload.reportFingerprint, "reportFingerprint"), boundary: b(payload.boundary) }, options); }
export function createDesktopPrepareTrainingDatasetApproveSuccessResponse(value: DesktopPrepareTrainingDatasetApproveSuccessValue, options?: { requestId?: string; correlationId?: string }): DesktopPrepareTrainingDatasetApproveResponse { return createIpcSuccessResponse(DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_RESPONSE_CHANNEL, value, options); }
...
export function getDesktopPrepareTrainingDatasetChannel(kind: "approve-request"): typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_REQUEST_CHANNEL;
export function getDesktopPrepareTrainingDatasetChannel(kind: "approve-response"): typeof DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_RESPONSE_CHANNEL;
export function getDesktopPrepareTrainingDatasetChannel(kind: "start-request" | "start-response" | "task-read-request" | "task-read-response" | "task-cancel-request" | "task-cancel-response" | "approve-request" | "approve-response"): DesktopPrepareTrainingDatasetChannel {
  switch (kind) {
    ...
    case "approve-request": return DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_REQUEST_CHANNEL;
    case "approve-response": return DESKTOP_DATASET_PREPARE_TRAINING_APPROVE_RESPONSE_CHANNEL;
