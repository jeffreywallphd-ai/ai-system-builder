...
export function useArtifactBrowserHuggingFace({
  ...
  const [localizedArtifact, setLocalizedArtifact] = useState<
    DesktopLocalizedArtifactFromRepo | undefined
  >();
  ...
  const [huggingFaceNamespaceDatasets, setHuggingFaceNamespaceDatasets] =
    useState<DesktopHuggingFaceNamespaceDataset[]>([]);
  ...
  const [tokenState, setTokenState] = useState<ArtifactBrowserViewState>({
    status: "idle",
  });
  const [huggingFaceTokenStatus, setHuggingFaceTokenStatus] = useState<{
    configured: boolean;
    maskedToken?: string;
  }>({
    configured: false,
  ...
  async function saveHuggingFaceToken(): Promise<void> {
    setTokenState({
      status: "loading",
      message: "Saving Hugging Face token...",
    });
    try {
  ...
  async function clearHuggingFaceToken(): Promise<void> {
    setTokenState({
      status: "loading",
      message: "Removing Hugging Face token...",
    });
    try {
      ...
      setTokenState({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Failed to remove Hugging Face token.",
      });
    }
  }
