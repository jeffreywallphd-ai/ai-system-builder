...
} from "../../../../../../../modules/contracts/runtime-installer";
...
function unavailable(method: string) {
  return {
    ...
    error: {
      code: "unavailable",
      message: `Desktop API method ${method} is unavailable.`,
    },
  };
}
...
export function createDesktopImageGenerationClient() {
  ...
  return {
    async startImageGeneration(
      ...
      if (!response.ok) {
        return {
          ...
          error: {
            ...
            message:
              response.error?.message ?? "Failed to start image generation.",
            details: response.error?.details,
          },
        };
      }
      const requestId = (response.value as { requestId?: string } | undefined)
        ?.requestId;
      if (typeof requestId !== "string" || requestId.trim().length === 0) {
        return {
          ...
          error: {
            code: "internal",
            message: "Image generation start response missing requestId.",
          },
        };
      }
    ...
    async readImageGeneration(
      input: { requestId: string },
      context?: { requestId?: string; correlationId?: string },
    ): Promise<ImageGenerationReadResult> {
      ...
      if (!isPreloadResponseEnvelope(response)) {
        return {
          ...
          error: {
            code: "internal",
            message: "Failed to read image generation status.",
          },
        };
      }
      if (!response.ok) {
        return {
          ...
          error: {
            ...
            message:
              ...
              "Failed to read image generation status.",
            details: response.error?.details,
          },
        };
      }
    ...
    async cancelImageGeneration(
      input: { requestId: string },
      context?: { requestId?: string; correlationId?: string },
    ): Promise<ImageGenerationCancelResult> {
      ...
      if (!isPreloadResponseEnvelope(response)) {
        return {
          ...
          error: {
            code: "internal",
            message: "Failed to cancel image generation task.",
          },
        };
      }
      if (!response.ok) {
        return {
          ...
          error: {
            ...
            message:
              response.error?.message ??
              "Failed to cancel image generation task.",
            details: response.error?.details,
          },
        };
      }
      return {
        ok: true,
        value: response.value as { cancelled: boolean; message?: string },
      };
    },

    async finalizeImageGenerationIfCompleted(
      input: { requestId: string; workspaceId?: string },
      context?: { requestId?: string; correlationId?: string },
    ): Promise<ImageGenerationFinalizeResult> {
      if (!api.finalizeImageGenerationIfCompleted) {
        return unavailable("finalizeImageGenerationIfCompleted");
      }
      const response = await api.finalizeImageGenerationIfCompleted(
        input,
        context,
      );
      if (!isPreloadResponseEnvelope(response)) {
        return {
          ...
          error: {
            code: "internal",
            message: "Failed to finalize image generation task.",
          },
        };
      }
      if (!response.ok) {
        return {
          ...
          error: {
            ...
            message:
              response.error?.message ??
              "Failed to finalize image generation task.",
            details: response.error?.details,
          },
        };
      }
      return {
        ok: true,
        value: response.value as DesktopImageGenerationFinalizeResult,
      };
    },

    async readComfyUiInstallStatus(context?: {
      requestId?: string;
      correlationId?: string;
    }): Promise<ComfyUiInstallStatusClientResult> {
      if (!api.readComfyUiInstallStatus)
        return unavailable("readComfyUiInstallStatus");
      const response = await api.readComfyUiInstallStatus({}, context);
      if (!isPreloadResponseEnvelope(response))
        return {
          ok: false,
          error: {
            code: "internal",
            message: "Failed to read ComfyUI install status.",
          },
        };
      if (!response.ok)
        return {
          ...
          error: {
            ...
            message:
              response.error?.message ??
              "Failed to read ComfyUI install status.",
            details: response.error?.details,
          },
        };
      if (!response.value || typeof response.value !== "object") {
        return {
          ...
          error: {
            code: "internal",
            message: "Malformed ComfyUI install status response.",
          },
        };
      }
      return {
        ok: true as const,
        value: response.value as RuntimeInstallStatusResult,
      };
    },

    async repairComfyUiInstall(context?: {
      requestId?: string;
      correlationId?: string;
    }): Promise<ComfyUiRepairInstallClientResult> {
      if (!api.repairComfyUiInstall) return unavailable("repairComfyUiInstall");
      const response = await api.repairComfyUiInstall({}, context);
      if (!isPreloadResponseEnvelope(response))
        return {
          ok: false,
          error: {
            code: "internal",
            message: "Failed to repair ComfyUI install.",
          },
        };
      if (!response.ok)
        return {
          ...
          error: {
            code: response.error?.code ?? "internal",
            message:
              response.error?.message ?? "Failed to repair ComfyUI install.",
            details: response.error?.details,
          },
        };
      if (!response.value || typeof response.value !== "object") {
        return {
          ...
          error: {
            code: "internal",
            message: "Malformed ComfyUI repair install response.",
          },
        };
      }
      return {
        ok: true as const,
        value: response.value as RuntimeInstallResult,
      };
    },
  };
}
