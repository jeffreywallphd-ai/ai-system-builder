...
- Desktop dataset preparation uses async **start/read/cancel task IPC** channels. The thin client uses the equivalent authenticated HTTP operation family under /api/dataset-preparation.
...
- Production dataset preparation path: ordered UI -> IPC or authenticated HTTP -> PrepareTrainingDatasetFromArtifactsUseCase -> RuntimeTaskRegistryPort -> Python runtime task-registry adapter -> /tasks/start and /tasks/{requestId}.
- The application use case records workspace ownership at start. Reads and cancellation from another workspace fail as not found.
- The runtime produces physical train, validation, and test outputs with deterministic source/group and exact-duplicate isolation.
